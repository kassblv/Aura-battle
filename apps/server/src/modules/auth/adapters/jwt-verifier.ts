...
export class JwtAccessTokenVerifier implements AccessTokenVerifier {
  ...
  async verify(token: string): Promise<VerifiedToken> {
    const payload = await this.jwt.verifyAsync<{ sub?: unknown; cv?: unknown }>(token);
    return {
      sub: typeof payload.sub === 'string' ? payload.sub : '',
      // La version des identifiants a l'emission : comparee a la base par
      // `FreshAccessTokenVerifier`.
      ...(typeof payload.cv === 'number' ? { cv: payload.cv } : {}),
    };
  }
}
