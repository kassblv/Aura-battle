...
- **Routes d'inventaire bornées par joueur** (`InventoryRateLimit`) : seau à jetons en mémoire, un par joueur et par route. `PUT /inventory/loadout` : rafale 12, puis 2 par seconde — le vestiaire équipe au toucher, et quatre touches par seconde tiennent près de six secondes sans attente. `POST /inventory/buy` : rafale 5, puis 1 par seconde. `GET /inventory` : rafale 10, puis 2 par seconde — le client relit à l'ouverture, après chaque `match:end` qui rapporte des pièces et après chaque défi réclamé ; revenir d'un match et réclamer tous ses défis d'affilée tient dans la rafale. Le jeton est pris après l'authentification et **avant** l'analyse du corps : un corps invalide ou un achat refusé coûte autant qu'un autre. Refus en 429 `RATE_LIMITED`, traduit par le client (« Doucement ! Réessaie dans un instant. »). Chaque écriture ne lit l'inventaire qu'une fois et le catalogue reste en mémoire (il ne change qu'au seed, joué avant le démarrage) : un équipement coûte quatre requêtes SQL. Les écritures d'un même joueur (achat, équipement) passent **en file** (`KeyedSerializer`) : chacune lit ce que la précédente a écrit, et le dernier état annoncé au match est celui de la dernière écriture — deux touchers rapides ne laissent plus un loadout périmé au match, et un équipement lancé pendant un achat voit l'effet acheté. La file est **bornée** (`INVENTORY_WRITE_QUEUE`), car la limite de débit borne le rythme, pas la profondeur : quatre écritures au plus par joueur, la cinquième refusée en 429 `RATE_LIMITED` ; une écriture qui n'a pas rendu la main 60 s après son début cède la file à la suivante (elle continue, seul l'ordre est relâché, et le journal le signale). Sans ces deux bornes, une requête pendante derrière une coupure réseau silencieuse bloquait le joueur pour toujours. Côté base, `pg` n'avait aucun délai par défaut ; ils sont posés dans `shared/database-timeouts.ts` : connexion 5 s, instruction 10 s (coupée par Postgres), réponse 12 s (coupée par le client), transaction muette 20 s. **Réserves :** la limite et la file vivent dans le processus (un seul conteneur, ADR 0012 ; à porter dans Redis au multi-nœud) ; comptée par joueur, elle ne borne pas un attaquant aux nombreux comptes invités, que seule la limite par IP de `/auth/device` freine.
