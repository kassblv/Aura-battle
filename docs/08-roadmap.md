...
- [x] Feature flags, bulle d'intention en test A/B (2026-09-26) : drapeaux déclarés en code, part par variable d'environnement (`FLAG_INTENT_BUBBLE_ROLLOUT`, 50 par défaut, figée pendant une mesure), affectation stable par joueur inscrite en base ; bulle en partie rapide et invitation quand tous les sièges réels sont exposés ; lecture par groupe dans `/admin/experiments`. Vérifiée dans un vrai duel à deux clients — spec `docs/superpowers/specs/2026-09-26-bulle-intention-ab-design.md`. **Reste :** l'IA du solo qui annonce et bluffe.
- [ ] Contenu servi avec `contentVersion` et cache client
- [x] Analytics produit (2026-09-26) : les sept indicateurs de `00-vision.md`, mesurés chez nous — aucun fournisseur, aucun traceur tiers. Presque tout se déduit des matchs et des comptes ; seul le partage de clip est un événement client (`POST /events`, protocole 2.5.0). Lecture dans le panneau d'administration (`GET /admin/indicators`), avec effectif et « échantillon insuffisant » sous 20 — spec `docs/superpowers/specs/2026-09-26-indicateurs-produit-design.md`.
